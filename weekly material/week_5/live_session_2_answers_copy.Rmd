...
vaccine_columns_1 <- read_csv(
  ...
  col_types = cols_only(county = col_character(), 
                        cumulative_booster_recip_count = col_double(), 
                        booster_eligible_population = col_double(), 
...
vaccine_columns_2 <- vaccine_full[ ,c("county", 
...
vaccine_columns_3 <- select(vaccine_full, c("county", 
           "cumulative_booster_recip_count", 
           "booster_eligible_population", 
           "administered_date"))
```
...
```{r}
# method 1: which
vaccine_rows_1 <- vaccine_columns_1[which(vaccine_columns_1$county %in% 
                       c("Los Angeles","Orange","Santa Barbara", "Ventura", 
                             "San Bernardino","Kern","San Luis Obispo",
                             "Imperial", "Riverside", "San Diego") & 
                        vaccine_columns_1$administered_date == "2022-09-15"), ]
vaccine_rows_1
```
...
vaccine_rows_2 <- subset(vaccine_columns_1, county %in% 
                           c("Los Angeles","Orange","Santa Barbara", "Ventura", 
                             "San Bernardino","Kern","San Luis Obispo",
                             "Imperial", "Riverside", "San Diego") &
                           administered_date=="2022-09-15")
vaccine_rows_2
```
...
vaccine_rows_3 <- filter(vaccine_columns_1, county %in% 
                           c("Los Angeles","Orange","Santa Barbara", "Ventura", 
                             "San Bernardino","Kern","San Luis Obispo",
                             "Imperial", "Riverside", "San Diego") &
                           administered_date=="2022-09-15") #as_date("2022-09-15")
vaccine_rows_3
```
...
```{r}
vaccine_rename <- rename(vaccine_rows_1, boosted= cumulative_booster_recip_count,
                                         eligible= booster_eligible_population)
vaccine_rename
```
...
```{r}
vaccine_perc <- mutate(vaccine_rename, perc_boosted= round((boosted/eligible)*100, 2))
vaccine_perc
```
...
```{r}
vaccine_arranged <- arrange(vaccine_perc, desc(perc_boosted))
vaccine_arranged

vaccine_arranged_2 <- arrange(vaccine_perc, -(perc_boosted))
vaccine_arranged_2
```
...
```{r}
vaccine_goal <- mutate(vaccine_arranged, goal_met=ifelse(perc_boosted>50,"Y","N"))
vaccine_goal
```
...
vaccine_tidy <- vaccine_full %>%
  ...
  filter(county %in% 
                           c("Los Angeles","Orange","Santa Barbara", "Ventura", 
                             ...
                             "Imperial", "Riverside", "San Diego") &
                           administered_date=="2022-09-15") %>%
  rename(boosted= cumulative_booster_recip_count,
                                         eligible= booster_eligible_population) %>%
  mutate(perc_boosted= round((boosted/eligible)*100, 2)) %>%
  mutate(goal_met=ifelse(perc_boosted>50,"Y","N")) %>%
  arrange(desc(perc_boosted))
vaccine_tidy 
```
