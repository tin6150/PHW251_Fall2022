...
# returning only those with e or o  (regexp syntax)
...
word_bag = c("apple", "mango", "pineapple")
...
result = word_bag[ str_detect(word_bag, "ap") ]
str(result)


fruit_df <- data.frame( fruit_name = word_bag )
fruit_df$fav <- ifelse(str_detect(fruit_df$fruit_name, "e") == T, "Yes", "Not")
...
str_replace_all("Lab test Result COLUMN#5", "[ |#]", "_") ##
```
...
str_detect(dirty_data, regex("Esu", ignore_case=T) )
...
str_detect(word_bag, regex("APP", ignore_case=T) )
