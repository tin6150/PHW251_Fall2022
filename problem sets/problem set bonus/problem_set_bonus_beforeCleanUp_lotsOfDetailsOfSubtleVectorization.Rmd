...
# going to assume that any height less than 8 is in ft.inches
...
# need a vector ready ifelse (if_else has some diff and compare to scalar didnt work as expected)
...
verNoReturn_calculate_bmi = function( height, weight ) {
  if_else( height >= as.vector(9),  
...
TMP2_calculate_bmi = function( height, weight ) {
  #str( height )
  #if( height >= 9 ) {    # 
  ifelse( height >= 9,
    return( "bmi_metric(   height,weight )" ),
...
```{r vectorization_not_handled_correctly, include=F}
# your code here

avg_colo = df_colorado %>% 
  #mutate( bmi = bmi_imperial(height, weight) ) %>%
  mutate( bmi = calculate_bmi(height, weight) ) %>%   # this works if vectorized ifelse() can somehow seriallly apply T vs F for each list item.
...
##++FIXME++
df_all = rbind( df_colorado, df_taiwan )
...
```{r vector_version_not_working, include=F }
# your code here
...
combo_df = combo_df %>% 
  #mutate( bmi = bmi_imperial(height, weight) ) #%>%
  #mutate( bmi = calculate_bmi(height, weight) )
  #mutate( bmi = subtlyIncorrectVector_calculate_bmi( height, weight   ) )
  mutate( bmi = verNoReturn_calculate_bmi( height, weight   ) )
...
doc_work_by_rank %>% arrange( num_visits ) %>%  head( 8 )  
...
formattable( 
  bll_wide,
  list(
    `2014` = formatter( "span", style = ~style( color = up_down(`2014`) ) )
  )
)
